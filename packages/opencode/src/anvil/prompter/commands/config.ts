...
interface CommanderCommand {
  ...
  hook(name: 'preAction', callback: (thisCommand: CommanderCommand) => void): CommanderCommand;
 ...
 */
export function registerConfigCommand(program: CommanderCommand): void {
  ...
  configCmd
    ...
    .action((key: string, value: string, options: { string?: boolean; allowUnknown?: boolean }) => {
      ...
      if (!keyValidation.valid && !allowUnknown) {
        ...
        console.error('Use "anvil config list" to see available keys.');
  ...
  configCmd
    ...
    .action(async (options: { all?: boolean; yes?: boolean }) => {
      if (!options.all) {
        ...
        console.error('Usage: anvil config reset --all [-y]');
        process.exitCode = 1;
        return;
      }
