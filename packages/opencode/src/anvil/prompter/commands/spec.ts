...
const SPECS_DIR = 'anvil/specs';

interface CommanderCommand {
  command(name: string): CommanderCommand;
  description(description: string): CommanderCommand;
  option(flags: string, description: string): CommanderCommand;
  hook(name: 'preAction', callback: () => void): CommanderCommand;
  action(callback: (...args: never[]) => unknown): CommanderCommand;
}
...
function filterSpec(spec: Spec, options: ShowOptions): Spec {
  ...
  const metadata = spec.metadata ?? { version: '1.0.0', format: 'anvil' as const };
...
export class SpecCommand {
  ...
  async show(specId?: string, options: ShowOptions = {}): Promise<void> {
    ...
    if (!existsSync(specPath)) {
      throw new Error(`Spec '${specId}' not found at anvil/specs/${specId}/spec.md`);
    }
    ...
    if (options.json) {
      ...
      const output = {
        ...
        metadata: parsed.metadata ?? { version: '1.0.0', format: 'anvil' as const },
      };
...
export function registerSpecCommand(rootProgram: CommanderCommand) {
  ...
  specCommand.hook('preAction', () => {
    console.error('Warning: The "anvil spec ..." commands are deprecated. Prefer verb-first commands (e.g., "anvil show", "anvil validate --specs").');
  });
